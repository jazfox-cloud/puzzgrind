...
The Production deploy command must never be the unqualified `pnpm exec opennextjs-cloudflare deploy`; without `--env production`, Wrangler selects the Preview-safe top-level configuration. `pnpm deploy` checks runtime configuration, cleans both build directories, builds once, and stages OpenNext's cache under the exact `assets/cdn-cgi/_next_cache` upload path. Its marker at `assets/cdn-cgi/puzzgrind/build-environment.json` records the environment, full Git SHA, Next build ID, critical-file digests, and the deployment cache policy. The artifact guard validates that marker plus the uploaded home, Privacy, robots, and sitemap cache entries before `deploy --env production` can run.

Dashboard settings are external state and are not changed by repository commits. Keep Production branch `main`, Build command `pnpm build:cloudflare:ci`, Deploy command `pnpm deploy`, and Version command `npx wrangler versions upload`. This removes the ambiguous Production double-build while preserving isolated PR Preview builds.

Next.js gives permanent static pages `s-maxage=31536000` by default, and OpenNext applies that shared-cache header to HTML pages without a finite revalidation period. The public CDN cache key for a standard URL does not include the Worker Version or Next Build ID, so a non-production artifact accidentally served on the Production hostname can remain after a deploy or rollback. PuzzGrind therefore uses the conservative shared-cache bypass strategy for `/`, `/privacy`, `/robots.txt`, and `/sitemap.xml`: browser-facing `Cache-Control: public, max-age=0, must-revalidate` plus `CDN-Cache-Control: no-store` and `Cloudflare-CDN-Cache-Control: no-store`.

OpenNext's default dummy incremental cache would ignore the verified prerender files and render a miss in the Worker. The configured static-assets incremental cache instead reads `cdn-cgi/_next_cache/<Next Build ID>/<route>.cache`, while webpack freezes the build environment into dynamic server code so a request-time binding cannot change an artifact's SEO identity. Fingerprinted `/_next/static/*` assets retain `public, max-age=31536000, immutable` through `public/_headers`.

The post-deploy smoke test requests both the unmodified standard home/Privacy URLs and Git-SHA cache-busted variants. It fails when their HTML differs, canonical/noindex signals diverge, a deployment-sensitive route has a long shared-cache directive, robots blocks the site, sitemap is incomplete, or fingerprinted assets lose immutable caching. A failed smoke is a failed release and requires immediate traffic rollback; cache purge is recovery assistance, not the correctness mechanism.
