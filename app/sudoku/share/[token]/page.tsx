...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const title = `Daily Sudoku in ${formatClockTime(result.durationSeconds)}`;
...
export default async function SudokuSharePage({ params }: Props) {
  ...
  if (!result) notFound();
  const shareUrl = siteUrl(`/sudoku/share/${token}`);
  const shareText = `I finished PuzzGrind Daily Sudoku in ${formatClockTime(result.durationSeconds)} with ${result.mistakes} mistakes and ${result.hintCount} hints. Can you beat it?`;
  return <main className="mx-auto min-h-screen max-w-3xl px-6 py-8 sm:px-10">
    ...
    <section className="py-14 text-center">
      ...
      <div className="mt-9 grid grid-cols-2 gap-3 sm:grid-cols-4">
        <div className="rounded-2xl bg-white/75 p-5"><span className="text-sm text-[var(--ink-soft)]">Time</span><strong className="mt-1 block text-2xl">{formatClockTime(result.durationSeconds)}</strong></div>
